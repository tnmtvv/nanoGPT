  ...
  for opt in "${OPTIMIZERS[@]}"; do
    for bs in "${BATCH_SIZES[@]}"; do
      for lr in "${LEARNING_RATES[@]}"; do
        ...
        CONFIG_FILE="config/shakespeare_char_original.py"
        ...
        if [ -f "$CONFIG_FILE" ]; then
          ...
          CUDA_VISIBLE_DEVICES=7 python train.py --config "${CONFIG_FILE}" --optimizer "${opt}" --learning_rate "${lr}" --batch_size "${bs}" --rank "${rank}" --wandb_run_name "${opt}_bs${bs}_lr${lr}_alpha${alpha}" --max_iters "5000"
        else
