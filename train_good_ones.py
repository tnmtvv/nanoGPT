...
# Fix: use named_parameters() and compare NAME strings, not Parameter objects
for name, p in model.named_parameters():
    if name not in good_layers:
        p.requires_grad = False
    else:
        p.requires_grad = True  # explicit, good practice

# optimizer
optimizer = model.configure_optimizers_good_layers(weight_decay, learning_rate, (beta1, beta2), device_type, opt_name=optimizer_name)
if init_from == 'resume':
